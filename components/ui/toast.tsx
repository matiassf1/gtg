...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      // Mobile: bottom of screen, above the empresa bottom nav (h-14 = 3.5rem)
      // Desktop: bottom-right corner
      "fixed z-[100] flex max-h-screen w-full flex-col gap-2 p-4",
      "bottom-16 left-0 right-0",
      "sm:bottom-4 sm:right-4 sm:left-auto sm:w-auto sm:max-w-[420px]",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      // Always visible — no opacity-0, critical for mobile (no hover state)
      "shrink-0 rounded-md p-1 text-foreground/60 transition-colors hover:text-foreground focus:outline-none focus:ring-2 focus:ring-ring",
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Description ref={ref} className={cn("text-sm opacity-90 leading-snug", className)} {...props} />
));
